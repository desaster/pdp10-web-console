...
import { KeyboardHandler, MOD_LSHIFT, MOD_LCTRL, MOD_LMETA, MOD_LTOP } from './keyboard';
...
class App {
  ...
  private initTV11(): void {
    ...
    this.keyboard = new KeyboardHandler((code) => this.tv11Connection.sendKey(code));
    ...
    document.addEventListener('fullscreenchange', () => setTimeout(() => this.updateTV11Scale(), 50));
  ...
  private updateCheatSheet(): void {
    ...
    const cheatType = this.activeTab === 'term0' ? 'terminal' : this.activeTab;
  ...
  private setupConnectButtons(): void {
    document.querySelectorAll('.terminal-titlebar .connect-btn').forEach((btn) => {
  ...
  private connect(target: TerminalType): void {
    switch (target) {
      case 'tv11':
        ...
        break;
      case 'console':
        this.consoleTerminal?.setPreviewMode(false, TERM_COLORS[this.currentColor]);
        this.consoleConnection?.connect();
        break;
      case 'term0':
        this.term0Terminal?.setPreviewMode(false, TERM_COLORS[this.currentColor]);
  ...
  private setTerminalColor(color: TermColor): void {
    ...
    this.consoleTerminal?.setPreviewMode(consolePreview, TERM_COLORS[color]);
