...
export class VirtualKeyboard {
    ...
    private createKeyElement(key: KLEKeyDef): HTMLElement {
        ...
        if (key.isIsoEnter) {
            ...
            if (key.w2 !== undefined)
                keyEl.style.setProperty('--key-w2', key.w2.toString());
            if (key.h2 !== undefined)
                keyEl.style.setProperty('--key-h2', key.h2.toString());
            if (key.x2 !== undefined)
                keyEl.style.setProperty('--key-x2', key.x2.toString());
        }
    ...
    private handleKeyUp(key: KeyDef): void {
        ...
        if (key.modifier) {
            ...
            for (const mod of this.activeModifiers) {
                const modKey = this.layout.kleKeys?.find(
                    (k) => k.modifier === mod
                );
    ...
    private updateModifierVisuals(): void {
        this.container.classList.toggle(
            'vk-shifted',
            this.activeModifiers.has('shift')
        );
        this.container.classList.toggle(
            'vk-ctrl',
            this.activeModifiers.has('ctrl')
        );
        this.container.classList.toggle(
            'vk-meta',
            this.activeModifiers.has('meta')
        );
        this.container.classList.toggle(
            'vk-top',
            this.activeModifiers.has('top')
        );
    }
